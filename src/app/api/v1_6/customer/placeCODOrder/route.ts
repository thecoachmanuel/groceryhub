...
export async function POST(req: NextRequest) {
  try {
    ...
    // Build items from body or from cart
    ...
    const totalAmount = Number(body.total_amount || subtotal + deliveryCharge - discount || 0);
    ...
    const newOrder = await Order.create({
      ...
      wallet_amount_used: Number(body.wallet_amount || 0),
      ...
      delivery_address: {
        ...
        address_line: typeof body.address === 'string' ? body.address : (body.delivery_address?.address_line || ''),
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        total_amount: totalAmount,
        delivery_pin: newOrder.delivery_pin,
      },
    });
  } catch (error: any) {
    console.error('placeCODOrder error:', error);
    return NextResponse.json(
      { status: 'error', result: 'false', message: error?.message || 'Failed to place order' },
      { status: 500 }
    );
  }
}
