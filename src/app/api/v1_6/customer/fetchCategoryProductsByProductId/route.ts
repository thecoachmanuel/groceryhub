...
export async function POST(req: NextRequest) {
  try {
    ...
    let products = await Product.find(query).sort({ rating: -1 }).limit(10).lean<any[]>();

    // Fallback if not enough category products
    if (products.length < 4) {
      products = await Product.find({ status: 'active', is_approved: true }).limit(10).lean<any[]>();
    }
