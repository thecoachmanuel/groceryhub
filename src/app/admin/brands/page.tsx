...
import { Tag, Plus, Search, Trash2, Edit3, X, RefreshCw } from 'lucide-react';
...
export default function AdminBrandsPage() {
  ...
  const [form, setForm] = useState({ name: '', logo: '', description: '', is_featured_on_home: false, status: 'Active' });
  ...
  const openCreateModal = () => {
    ...
    setForm({ name: '', logo: '', description: '', is_featured_on_home: false, status: 'Active' });
    setShowModal(true);
  };

  const openEditModal = (b: BrandItem) => {
    setEditingBrand(b);
    setForm({ name: b.name, logo: b.logo, description: b.description, is_featured_on_home: b.is_featured_on_home, status: b.status });
    setShowModal(true);
  };
  ...
  const handleToggleFeatured = async (id: string, current: boolean) => {
    try {
      await fetch('/api/admin/brands', {
        ...
        body: JSON.stringify({ brandId: id, id, is_featured_on_home: !current }),
      });
  ...
  const handleToggleStatus = async (id: string, current: string) => {
    ...
    try {
      await fetch('/api/admin/brands', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brandId: id, id, status: newStatus }),
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch('/api/admin/brands', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brandId: id, id }),
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingBrand ? { ...form, id: editingBrand.id, brandId: editingBrand.id } : form;
      const res = await fetch('/api/admin/brands', {
        method,
        headers: { 'Content-Type': 'application/json' },
      ...
      if (json.success) {
        setShowModal(false);
        fetchBrands();
      } else { alert(json.message || 'Failed to save brand'); }
    } catch (err) { console.warn(err); }
    setSaving(false);
  };
      ...
      <main className="flex-1 p-8 space-y-6 overflow-y-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Certified partner brands shown on frontend /sellers and /brands pages, and Expo app filters</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center space-y-3">
            ...
            <p className="text-xs text-gray-400">No brands match your query.</p>
          </div>
        ) : (
              ...
              <div key={b.id} className="bg-[#1e2632] border border-gray-800 rounded-2xl p-4 space-y-3 hover:border-gray-700 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-500">/{b.slug}</p>
                  </div>
                </div>
                {b.description ? (
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEditModal(b)} className="p-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-blue-400 transition-all" title="Edit Brand">
                    <Edit3 size={13} />
                  </button>
                  ...
                  <button onClick={() => handleDelete(b.id)} className="p-1.5 rounded-lg bg-red-950/30 hover:bg-red-950/60 text-red-400 transition-all" title="Delete Brand">
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e2632] w-full max-w-md rounded-3xl p-6 border border-gray-800 space-y-4 relative">
            <button onClick={() => setShowModal(false)} className="absolute right-5 top-5 p-2 rounded-full hover:bg-gray-800 text-gray-400"><X size={20} /></button>
            <h3 className="text-xl font-black">{editingBrand ? 'Edit Brand Details' : 'Add New Brand'}</h3>
            <p className="text-xs text-gray-400">Update logo, title, description, and status for web & app.</p>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-300">Brand Name *</label>
                <input type="text" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Dangote Foods"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-300">Brand Logo / Image Upload</label>
                <LocalImageUploader
                  value={form.logo}
              ...
              <div className="space-y-1">
                ...
                <select value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value }))} className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl p-2.5 text-xs focus:outline-none focus:border-[#0aad0a]">
